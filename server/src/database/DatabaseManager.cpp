...
DatabaseManager::DatabaseManager()
{
    ...
    for (auto &p : syncSchemaRes) {
        std::cerr << "Database sync schema: " << p.first << " " << p.second
                  << std::endl;
    }
}
...
void DatabaseManager::dumpDatabase() const
{
    ...
    for (auto &user : _storage->iterate<User>()) {
        std::cout << user.id << "\tusername: " << user.username
             << "\tip: " << user.ip << "\tport: " << user.port << std::endl;
    }
    std::cout << "Contact List:" << std::endl;
    for (auto &contact : _storage->iterate<Contact>()) {
        std::cout << contact.id << "\tuser id: " << *contact.userId
                  << "\tcontact user id: " << *contact.contactId << std::endl;
    }
}
...
void DatabaseManager::insertTestDataset()
{
    try {
        ...
        this->_storage->replace(Contact{
            1, std::make_unique<int>(1), std::make_unique<int>(2)}); // 1 => 2
        this->_storage->replace(Contact{
            2, std::make_unique<int>(1), std::make_unique<int>(3)}); // 1 => 3
        this->_storage->replace(Contact{
            3, std::make_unique<int>(3), std::make_unique<int>(1)}); // 3 => 1
        this->_storage->replace(Contact{
            4, std::make_unique<int>(3), std::make_unique<int>(2)}); // 3 => 2
    } catch (std::exception const &e) {
        std::cerr << "DatabaseManager::insertTestDataset() " << e.what()
                  << std::endl;
    }
}

void DatabaseManager::setUser(
    std::string const &username, std::string const &ip, std::size_t port)
{
    try {
...
std::vector<User> DatabaseManager::getContacts(std::string const &username)
{
    ...
    auto rows = _storage->select(
        columns(&Contact::contactId, &User::username, &User::ip, &User::port),
...
void DatabaseManager::newContact(
    std::string const &username, std::string const &contactName)
{
    const User user = this->getUser(username);
    const User contact = this->getUser(contactName);

    Contact data = {0, std::make_unique<int>(user.id),
        std::make_unique<int>(contact.id)};
    _storage->insert(data);
}

void DatabaseManager::removeContact(
    std::string const &username, std::string const &contactName)
{
    const User user = this->getUser(username);
    const User contact = this->getUser(contactName);

    _storage->remove_all<Contact>(where(c(&Contact::userId) == user.id
        and c(&Contact::contactId) == contact.id));
}

void DatabaseManager::newUser(
    std::string const &username, std::string const &ip, std::size_t port)
{
...
void DatabaseManager::updateUser(
    std::string const &username, std::string const &ip, std::size_t port)
{
